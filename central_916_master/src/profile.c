...
const uint8_t UUID_CONSOLE[]        = {0x43,0xF4,0xB1,0x14,0xCA,0x67,0x48,0xE8,0xA4,0x6F,0x9A,0x8F,0xFE,0xB7,0x14,0x6A};
...
static void output_notification_handler(uint8_t packet_type, uint16_t _, const uint8_t *packet, uint16_t size)
{
    ...
    case GATT_EVENT_NOTIFICATION:
        ...
        platform_printf("Receive from slave:");
        printf_hexdump(&value_packet->value, value_size);
    
    
        gatt_client_write_value_of_characteristic_without_response(slave.conn_handle,
                                                                   slave.input_char.value_handle,
                                                                   value_size, (uint8_t*)&value_packet->value);
        break;
    }
}
...
{
    platform_printf("Package num per second:%d\r\n", sum_per_second);
...
static void app_timer_callback(TimerHandle_t xTimer)
{
   ...
   // btstack_push_user_msg(USER_MSG_LOOPBACK_SEND, NULL, 0);
    //platform_printf("app_timer_callback");
...
bd_addr_t peer_addr_reverse = {0x01, 0x23, 0x45, 0x67, 0x89, 0xFB};
        ...
        case HCI_SUBEVENT_LE_EXTENDED_ADVERTISING_REPORT:
            {
                ...
                //if((report->address[0]==1)|(report->address[0]==7))
                {

                    //LOG_MSG("%d,%d %d %d",report->address[0],report->address[1],report->address[2],report->address[3]);

                }

                //printf_hexdump(report->address, 6);
             
                if (memcpy((uint8_t*)report->address, peer_addr, 6) == 0 || 
                    memcpy((uint8_t*)report->address, peer_addr_reverse, 6) == 0 ||
                    ad_data_contains_uuid128(report->data_len, report->data, UUID_TPT))
                {
...
uint32_t setup_profile(void *data, void *user_data)
{
    ...
    TimerHandle_t timer = xTimerCreate("t2",
                            pdMS_TO_TICKS(1000),
                            pdTRUE,
                            NULL,
                            timer_task);
